...
do_install () {
	install -d ${D}${libdir}/
	install -m 0644 ${WORKDIR}/usr/lib/chromium-browser/libwidevinecdm.so ${D}${libdir}/
	install -m 0644 ${WORKDIR}/usr/lib/chromium-browser/libwidevinecdmadapter.so ${D}${libdir}/
}

WARN_QA_remove = "ldflags already-stripped"
ERROR_QA_remove = "ldflags already-stripped"
